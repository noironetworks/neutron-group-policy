...
class ResourceMappingSGTestCase(
    ...
    def setUp(self):
        config.cfg.CONF.set_override('policy_drivers',
                                     ['implicit_policy', 'resource_mapping'],
                                     group='group_policy')
        super(ResourceMappingSGTestCase, self).setUp(
                                     core_plugin=SG_CORE_PLUGIN)
...
class TestContract(ResourceMappingSGTestCase):
    ...
    def test_endpoint_group_update(self):
        ...
        # now test EP update
        ep1_id = ep1['endpoint']['id']
        data = {'endpoint': {'endpoint_group_id': epg1_id}}
        req = self.new_update_request('endpoints', data, ep1_id)
        res = req.get_response(self.ext_api)
        self.assertEqual(res.status_int, webob.exc.HTTPOk.code)
